...
// (^)comecem com string (\) a primeira \ foi pra escapar (\d) pegar as strings que comecem com numeros g(globalmente) m(multilinhas)
...
function hasClass(markup, cssClass) {
    // tudo que iniciar com 'class='
    var regex = new RegExp('class=["\'](?:[\\w\\s]+)?' + cssClass + '(?:["\\w\\s]+)?["\']');
    return regex.test(markup); // vai retornar true ou false
}
